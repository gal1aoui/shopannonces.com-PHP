...
if(is_post_back()) {
	...
	if(is_array($arr_pd_ids)){
		$str_pd_ids = implode(',', $arr_pd_ids);
		 ...
		 {		  
			 $sql = "update ".DB.".tbl_enquiry set job_status  = 'Y' where rID   in ($str_pd_ids)";			
			 db_query($sql);
			 $_SESSION['site_admin_message']="jobs Activate Successfully.........";
		 }
		 else if(isset($_REQUEST['Deactivate']) || isset($_REQUEST['Deactivate_x']) )
		 {
			$sql = "update ".DB.".tbl_enquiry set job_status  = 'N' where rID    in ($str_pd_ids)";
			db_query($sql);
			 $_SESSION['site_admin_message']="jobs Deactivate Successfully.........";
	     }	
		 		
		 else if(isset($_REQUEST['Delete']) || isset($_REQUEST['Delete_x']))
		 {
				$selProduct=db_query("delete from tbl_enquiry  where rID in ($str_pd_ids)");
				//@unlink($file_path);  
				$_SESSION['site_admin_message']="Enquiry Deleted successfully........";			
			}
...
$columns = "select * ";
$sql = " from ".DB.".tbl_enquiry ";
$sql .= " where 1 ";
$order_by == '' ? $order_by = DB.'.tbl_enquiry.rID  ' : true;
...
?>
<?php echo PageTitle('Manage Enquiry');?>
  	...
  	<td align="center"><br />   
	    ...
	    <tr>
				...
				<tr class="<?php echo $css?>">
				  ...
				  <td nowrap="nowrap" align="center"><a href="javascript:poptastic('enquiry_detail.php?rID=<?php echo $rID ; ?>');"><b><font color="#FF0000"> view details/send reply </font></b></a> </td>
