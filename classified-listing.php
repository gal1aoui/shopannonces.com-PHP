...
if($_REQUEST['sort_by']!=""){     
	$sort=secureValue($_REQUEST['sort_by']);	
	if($sort=="new"){
	  $cond=" $cond_all "; 
...
if($keyword=='' || $keyword=='mot(s) clé(s)')
	$keyword='';
	
if($_REQUEST['catId']!=""){
 $id1 = intval($_REQUEST['catId']);
 $res_condtion =" clsd_cat_id ='$id1' $cond ";
}
if($_REQUEST['subcatId1']!=""){
 $id2 = intval($_REQUEST['subcatId1']);
 $res_condtion =" clsd_subcat_id ='$id2' $cond ";
}
if($_REQUEST['subcatId']!=""){
 $id3 = intval($_REQUEST['subcatId']);
 $res_condtion =" clsd_sub_subcat_id ='$id3' $cond "; 
}
if($_REQUEST['cityId']!=""){
	$id4 = intval($_REQUEST['cityId']);
	$res_condtion =" classified_city_id ='$id4' $cond ";
}
if($_REQUEST['classi_state']!=""){
	$id5 = intval($_REQUEST['classi_state']);
	if($res_condtion=='')
		$res_condtion ="classified_poster_state ='$id5' $cond ";
	else
		$res_condtion .="AND classified_poster_state ='$id5' $cond ";
}
...
if($_REQUEST['offer_type']!=""){
	$id6 = secureValue($_REQUEST['offer_type']);
	if($res_condtion=='')
		$res_condtion ="classified_type ='$id6' $cond ";
	else
		$res_condtion .="AND classified_type ='$id6' $cond ";
}
//---------------------------
...
//$sql=($fromannee==0 && $toannee==0 && $fromkm==0 && $tokm==0)? " FROM tbl_classified" : " FROM tbl_classified, tbl_option_cat, tbl_classified_other";
	...
	if($prix!=""){
		...
		$sql.=" ORDER BY trie ASC, `date_fin_republication` DESC LIMIT $start, $pagesize";
	}


//if(!empty($sql) && !empty($res_condtion)) {
...
if($_REQUEST['catId']!=""){
	...
	$paid_banner=manage_paidbanner($id);
}
if($_REQUEST['subcatId1']!=""){
	$id=intval($_REQUEST['subcatId1']);
	$paid_banner=manage_paidbanner($id);
}
if($_REQUEST['subcatId']!=""){
	$id=intval($_REQUEST['subcatId']);
	$paid_banner=manage_paidbanner($id);
}
*/
		...
		<?php
            ...
            if($main_catId!="" && $main_catId>0){?>
                <div style="padding:5px;">
                    <?php
                    echo manage_banner_requests($main_catId,"Top Banner");
                    ?>
                </div>
            <?php
            }			 
			...
			<div class=""> 
					...
					if($reccnt > 0 ) {
						...
						while($rw=mysql_fetch_array($rs_classi)){ 
							...
							$sql_img="select * from tbl_classified_image 
							where tbl_classified_image.clsd_id=$rw[classified_id] and img_status='Y'";
							...
							if(mysql_num_rows($sql_img_set) > 0 ) { 
								...
								if($res1[cls_img_file]!="" && file_exists($file_sm_root) ){
									...
									$im_small='<img src="'.$file_path_sm.'" alt="'.$alt.'" border="0" width="'.$width.'" height="'.$height.'" class="border-img"/>';
								}else{
                                	...
                                	<tr>
                                    	<td align="center" style="width:95px;"> 
                        					<div style="padding:10px; text-align:center;">                             
												<?=$AnnoncePremium=(strtotime($date_comp) < strtotime($rw[date_fin_premium])) ? '<span class="premium">PREMIUM</span>' : '';?>
                                                <?php                                    
                        	...
                        	<div style="padding:10px; text-align:center;">
											<?php
                                    $date_affichee=(strtotime(MYSQL_DATE_TIME) <= strtotime($rw[date_fin_republication])) ? 
													MYSQL_DATE_TIME : 
													$rw[classified_post_date]; 
                                    ?>
					...
					?>
						<div class="" >
                        	<strong>Aucune annonce dans cette catégorie....</strong>
                        </div>
					<?php
					}
					?>
