...
if(isset($_REQUEST['pcatId']) && $_REQUEST['pcatId']!=""){
	...
	{// onclick='cat_drop_down3(this.value,\"\")'
		$var.="<input name='cat_level_two' id='cat_level_two".$i."' value='".$rw_sc['cat_id']."' type='radio' required='required'>
...
if(isset($_REQUEST['osubcatId']) && $_REQUEST['osubcatId']!=""){
	$subCatID = intval($_REQUEST['subcatId']);
	  ...
	  if($num > 0 ){
			...
			while($rw=mysql_fetch_array($sql_cat)){ 
				$var.='
					<div><label class="select-label">'.$rw['option_nom'].'</label>';
					//------------------------------
					  ...
					  $num2=mysql_num_rows($sql);
					 	...
					 	if($num2 > 0 ){
							...
							$var.="<textarea rows='3' cols='30' name='".$rw[option_id]."' maxlength='60' class='input-textarea'></textarea>";
						}
						else if($rw['option_nom']==htmlentities("Année", ENT_QUOTES, "UTF-8")){
							//-----------
							
								$var.="<select name='".$rw['option_id']."' />"; 
								$var.='<option value="">Choisir une option</option>';	
								$ann=date("Y");
								while($ann>=1900){ 
						...
						$sql_fonction=db_query("select * from tbl_category where cat_parent ='793' and cat_status='Y' order by cat_order");
						   	...
						   	$var.="<select name='".$rw[option_id]."'
									class='input-text ui-input-text ui-body-c ui-corner-all ui-shadow-inset'  />"; 
									   $var.= '<option value="">Choisir une option</option>';
							...
							$var.="<input type='text' name='$rw[option_id]' value='' 
									class='input-text ui-input-text ui-body-c ui-corner-all ui-shadow-inset' 
									maxlength='10' maxlength='60' required='required' >";
						}
...
if(isset($_REQUEST['statId']) && $_REQUEST['statId']!=""){		
	...
	$sbCatID = intval($_REQUEST['sbcatId']);
	  ...
	  $num=mysql_num_rows($sql_cat);
	  $var='<select name="classi_city" id="classi_city" class="textbox" />';
...
if(isset($_REQUEST['statIdsearch']) && $_REQUEST['statIdsearch']!=""){
	...
	$sbCatID = intval($_REQUEST['sbcatId']);
	  ...
	  $var='<select name="cityId" id="classi_city" class="input-text" />';
...
if(isset($_REQUEST['saveimage']) && $_REQUEST['saveimage']!=""){
	$saveimage=$_REQUEST['saveimage'];
	
	//$_SESSION['sess_admin_login_id']=$saveimage;

	if(!isset($_SESSION['forms'][image]))
		$_SESSION['forms'][image] = array();
		
	$_SESSION['forms'][image][] = $saveimage;
