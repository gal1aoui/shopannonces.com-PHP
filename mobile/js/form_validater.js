...
function readURL_mobile(input,pos)
{
		alert("jjj");
		...
		if (files.length > 0) {
			...
			for ($i = 1; $i <= 7; $i++) {
				...
				$image_preview = $('#image_preview'+$i);
				
				if($image_preview.find('img').attr('src') == ""){
					...
					*/
					...
					var url	=	"ajax_respons.php?saveimage='"+window.URL.createObjectURL(file)+"'";
...
function cat_drop_prin_select_mobile(catID,scatId)
{
	document.getElementById("ogiga").innerHTML="";	
		var url	=	"ajax_respons.php?pcatId="+catID;
...
function cat_drop_down3_mobile(subcatID,scatId)
{
	document.getElementById("omega").innerHTML="";	
	var url	=	"ajax_respons.php?osubcatId="+subcatID+"&osbcatId="+scatId+"&ran="+Math.random();
	xmlHttp22	=	GetXmlHttpObject();
...
function cat_drop_down_search(subcatID,scatId)
{
	...
	var url	=	"ajax_respons.php?searchsubcatId="+subcatID+"&searchsbcatId="+scatId+"&ran="+Math.random();
	xmlHttp2	=	GetXmlHttpObject();	
	xmlHttp2.open("GET",url,true)
	xmlHttp2.send(null)	
	xmlHttp2.onreadystatechange = function()
...
{
	var url	=	"ajax_respons.php?catIdsearch="+catID+"&sbcatIdsearch="+scatId+"&ran="+Math.random();
...
function get_city_by_state_mobile(stateID,scatId)
{
	...
	{		
		...
		{		
			...
			if(txt!="")
			{
				...
				$("#bloc_city").removeClass().addClass(" ui-btn-inner ui-btn-corner-all ui-btn ui-shadow ui-btn-corner-all ui-btn-icon-right ui-btn-up-c" );
...
function serach_city_by_state(stateID,scatId)
{
	...
	{		
		...
		{		
			...
			if(txt!="")
			{
				document.getElementById("classi_city").innerHTML=xmlHttp3.responseText;
			}	
		}
	}
}
...
$(function () {
	...
	*/
	...
	$('#fileupload').find('input[name="image"]').on('change', function (e) {
		alert("jjj");
		var files = $(this)[0].files;
