...
function connect_db()
{
	...
	if (!isset($GLOBALS['dbcon'])) {	
		...
		mysql_set_charset('utf8',$GLOBALS['dbcon']); 
		mysql_select_db("clfly") or die(mysql_error());
	}
}
...
function db_query($sql, $dbcon2 = null)
{
	...
	$result	= mysql_query($sql,	$dbcon2) or	die(mysql_error());
	/*
...
{
//	$unit	=	searchSingleRecord("config","conf_unit","conf_name","shipping_charges"); 
...
function datetime_format($date)
{
	...
	if (strlen($date) >= 10) {
		...
		return date("m/d/Y h:i A ", $mktime);
	} else {
		return $s;
	}
}

function datetime_format1($date)
{
	global $arr_month_short;
	if (strlen($date) >= 10) {
		if ($date == '0000-00-00 00:00:00' || $date	== '0000-00-00') {
		...
		$mktime	= mktime(substr($date, 11, 2), substr($date, 14, 2), substr($date, 17, 2),substr($date,	5, 2), substr($date, 8,	2),	substr($date, 0, 4));
		return date("n/j/Y G:i:s", $mktime);
	} else {
		return $s;
	}
}
...
function make_dropdown($sql, $combo_name, $sel_value =	'',	$extra = '', $choose_one = '')
{
	...
	if (mysql_num_rows($result)	> 0) {
	  ...
	  echo "<select name='$combo_name' id='$combo_name' $extra><option value=''>$choose_one</option></select>";
	}
	return $str_dropdown;
}
...
function array_dropdown1( $arr, $sel_value='', $name='', $extra='', $choose_one='', $arr_skip= array())
{
	asort($arr);
	reset($arr);
	...
	foreach($manutmp as	$key =>	$value)	{
		...
		if ($key !=	$missit) {
			if ($value != "") {
				...
				$checkstr .= "<td valign='top'><INPUT TYPE='checkbox' $javascript	 NAME='$checkname" . '[]' .	"' value='$key'	$tochecked ></td><td $style valign='top' width='50%'> $value	</td>\n";
				$j++;
			}
		}
	}
...
function month_dropdown_1($name,	$selected_date_month = '', $extra='')
{
	global $ARR_MONTHS;
	...
	foreach ($ARR_MONTHS as $key => $value) {
		...
		$date_dropdown	.= " value='" .	str_pad($key, 2, "0",	STR_PAD_LEFT) .	"'>".str_pad($key, 2, "0",	STR_PAD_LEFT)."</option>";
	}
...
function sort_arrows($column)
{
	...
	return '
	<a href="' . $_SERVER['PHP_SELF'] .	get_qry_str(array('order_by', 'order_by2'),	array($column, 'asc')) . '">
		<img src="images/up_arrow.gif" border="0">
	</a>
		
	<a href="'	. $_SERVER['PHP_SELF'] . get_qry_str(array('order_by', 'order_by2'), array($column,	'desc')) . '">
...
function show_thumb($file_org, $width, $height, $ratio_type = 'width')
{
	...
	if(!is_file(THUMB_CACHE_DIR."/".$cache_file)) {
		make_thumb_gd($file_fs_path,THUMB_CACHE_DIR."/".$cache_file, $width, $height, $ratio_type );
	}else{
	    @unlink(THUMB_CACHE_DIR."/".$cache_file);
		make_thumb_gd_1($file_fs_path, THUMB_CACHE_DIR."/".$cache_file, $width, $height, $ratio_type );
	}	
	return "thumb_cache/".$cache_file;
}


function make_thumb_gd_1($imgPath, $destPath, $newWidth, $newHeight, $ratio_type = 'width', $quality = 60, $verbose = false)
{ 
	// options for ratio type = width|height|distort|crop
	// get image info (0 width and 1 height, 2 is (1 = GIF, 2 = JPG, 3 = PNG)
	$n=	$newWidth;
...
// show_thumb: Updated 6 feb 2007
function show_thumb_1($file_org, $width, $height, $ratio_type = 'width')
{
	...
	if(!is_file(SITE_FS_PATH."/".THUMB_CACHE_DIR."/".$cache_file)) {
		make_thumb_gd_1($file_fs_path, SITE_FS_PATH."/".THUMB_CACHE_DIR."/".$cache_file, $width, $height, $ratio_type );
	}else{
	    @unlink(SITE_FS_PATH."/".THUMB_CACHE_DIR."/".$cache_file);
		make_thumb_gd_1($file_fs_path, SITE_FS_PATH."/".THUMB_CACHE_DIR."/".$cache_file, $width, $height, $ratio_type );
	}	
