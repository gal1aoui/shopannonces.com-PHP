...
function chk_user_login(){
	...
	if(!empty($_SESSION['signin']['mem_id'])
		and !empty($_SESSION['signin']['user_id'])
		and !empty($_SESSION['signin']['lname'])
		and !empty($_SESSION['signin']['type'])
		and !empty($_SESSION['signin']['email']))
	{
		//si connecté ne rien faire
		//echo "connecté";
		return true;
	}
	else{
...
function Signin_member($user_id, $user_password, $ref){
...
function Register_member($user_id, $mem_lname, $mem_type, $mem_tel, $user_password, $confirm_password){
	...
	if( isset($user_id) && isset($mem_lname)  && isset($mem_type)  && isset($mem_tel)  && isset($user_password)  && isset($confirm_password) && ($user_password==$confirm_password) )
	{
...
function getCat_parent($cat_parent){
	if($cat_parent!=""){
		$cat_parent = secureValue($cat_parent);
		$re1=db_query("select * from tbl_category where cat_id ='$cat_parent'");
		$line_raw = mysql_fetch_array($re1);
		$cat_level_root=$line_raw["cat_parent"];
	}
return $cat_level_root;
}
...
function manage_banner_requests($catId,$position){
	...
	$var="r";
				$img=$res['image'];
				$file_path="../advertise/".$img;
				$file_path_root = "../advertise/".$img;
	...
	if($catId!="" && $position!=""){  
		...
		if(mysql_num_rows($rw)> 0 ) {
			while($res=mysql_fetch_array($rw)){
				$img=$res['image'];
				$file_path='../advertise/'.$img;
				$file_path_banner_root ='../advertise/'.$img;
...
function unlink_file_mobile($folder_name,$filename){	
	$folder=$folder_name;
...
function upload_file_mobile($mid, $image, $folder){
/*	
	if($nameFile!=""){
		
		$extension=getExtension($nameFile);
		
		if($extension=="jpg" || $extension=="JPG" || $extension=="jpeg" || $extension=="JPEG" || $extension=="gif" || $extension=="GIF" || $extension=="png" || $extension=="PNG"){
			
			$image_name=$mid.mt_rand(1000,9999).".".$extension;
			
			$NotVail_files = file_validation('image',$nameFile);
			
			if($image_name!="" && $NotVail_files){	
				@rename($nameFile, $folder.$image_name);
								
				$size = GetImageSize( $folder.$image_name );
				$width = $size[ 0 ];
				$height = $size[ 1 ];
				...
				if($width>$height){
					$dest_w = 800;
					$dest_h = $height/($width/$dest_w);
				}
				else if($width<$height){
					$dest_h = 800;
					$dest_w = $width/($height/$dest_h);
				}
				
				$miniature = ImageCreateTrueColor( $dest_w, $dest_h);
				$image = ImageCreateFromJpeg( $folder.$image_name );
				ImageCopyResampled( $miniature, $image, 0, 0, 0, 0, $dest_w, $dest_h, $width, $height );
				ImageJpeg( $miniature, $folder.$image_name, 100 );
			}
		 }else{
...
function upload_file_mobile_tempo($indeximage, $folder){	
/*
	if($_FILES['files']['name'][$indeximage]!=""){ 
	
		$uploadedfile=$_FILES['files']['tmp_name'][$indeximage];
		$extension=getExtension($_FILES['files']['name'][$indeximage]);		
		
		if($extension=="jpg" || $extension=="JPG" || $extension=="jpeg" || 
			$extension=="JPEG" || $extension=="gif" || $extension=="GIF" || $extension=="png" || $extension=="PNG"){
				
			$image_name=mt_rand(1000,9999).".".$extension;
			$tmp_imagename=$_FILES['files']['name'][$indeximage];
			$NotVail_files = file_validation('image',$uploadedfile);
			
			if($tmp_imagename!="" && $uploadedfile!="" && $image_name!="" && $NotVail_files){
...
function front_navigation($catId,$subcatId1="",$subcatId="",$clsId="",$stateId=""){
	...
	$nav='<div class="tree">'.
			'<a href="index.php" class="link1 b">Accueil</a>'.' >> '.$res1
		.'</div>';
	}
	if($catId=="" && $subcatId1!=""){
	 ...
	 $nav='<div class="tree">'.
			'<a href="index.php" class="link1 b">Accueil</a> >> 
...
Merci d'utiliser Flyannonces,<br />

L'équipe Flyannonces<br />
					</td>
				</tr>
			</tbody>
		</table>
...
function ajouter_facture($mode_paiement, $facture_total, $classified_id){
	...
	class PDF extends FPDF
	{
		...
		function Header()
		{
			...
			//$this->Cell(80);
			//Titre    
			$this->Image('./uploaded_files/logo/logo.png');//$this->Cell(30,10,'Mon joli fichier PDF',0,0,'C');
			//Saut de ligne
			$this->Ln(20);
		}
		...
		function Footer()
		{
			...
			$this->Cell(0,10,'Email : contact@flyannonces.com - web : www.flyannonces.com');
		}
	}
	...
	$pdf->Cell(0,10,$_SESSION['signin']['email']);
	$pdf->ln();
	...
	//$pdf->Output();
	$pdf->Output("./uploaded_files/factures/".$nouveau_num_facture.".pdf");
...
http://www.flyannonces.com/bill.php?reference=".$nouveau_num_facture."&mode=html


Pour la version pdf:

http://www.flyannonces.com/bill.php?reference=".$nouveau_num_facture."&mode=pdf
...
Cordialement.";
	...
	sent_mail($_SESSION['signin']['email'], "FACTURE ".$nouveau_num_facture, $body);
	
return $facture_id;
}


function subsub_cat($osubsubcatId, $osubcatId){
if( $osubcatId !="" && $osubsubcatId !="" ){
	$var='';
	$sql_sous_cat=db_query("select * from tbl_category where cat_parent ='".$osubcatId."' AND`cat_status`='Y' order by cat_order");
	$i=1;
	while($rw_sc=mysql_fetch_array($sql_sous_cat))
	{
		$selected=$osubsubcatId ==$rw_sc['cat_id'] ? "checked" : "";
		
	$var.="<input type='radio' name='cat_level_two' id='cat_level_two".$i."' value='".$rw_sc['cat_id']."' ".$selected." onclick='cat_drop_down3(this.value,\"\")' required='required'>
			<label for='cat_level_two".$i."'>"
			.$rw_sc['cat_name'].
			"</label><br>";
			$i++;
	}
...
function option_subsub_cat($osubcatId){
	
	$osubcatId = intval($osubcatId);
	...
	$num=mysql_num_rows($sql_cat);
	  ...
	  if($num > 0 ){
			...
			while($rw=mysql_fetch_array($sql_cat)){ 
				$var.='
					<div><label class="select-label" for="'.$rw[option_id].'">'.$rw['option_nom'].':</label>';
					//------------------------------
					  $sql=db_query("select * from tbl_value_option where option_id ='$rw[option_id]' ORDER BY val_id");
					  $num2=mysql_num_rows($sql);
					  
					 	if($num2 > 0 ){
							$var.="<select name='".$rw[option_id]."' id='".$rw[option_id]."' class='textbox' data-native-menu='false' required />"; 
							$var.='<option value="">Choisir une option</option>';	
							while($rw2=mysql_fetch_array($sql)){
								$selected = $_SESSION['forms'][CFG_STAGE_ID][$rw[option_id]] ==$rw2[val_id] ? "selected" : "";
								
								$var.='<option value="'.$rw2[val_id].'" '.$selected.' >'.$rw2['val_nom'].'</option>';
							}
							$var.="</select>";
						}
						else if($rw['option_nom']==htmlentities("Présentation de l'entreprise", ENT_QUOTES, "UTF-8")){
							$var.="<textarea rows='3' cols='30' name='".$rw[option_id]."' id='".$rw[option_id]."' maxlength='60' class='textbox' required='required'></textarea>";
						}
							...
							$var.="<select name='".$rw['option_id']."' id='".$rw[option_id]."' class='textbox' data-native-menu='false' required />"; 
								...
								$ann=date("Y");
								while($ann>=1900){
									$selected = $_SESSION['forms'][CFG_STAGE_ID][$rw[option_id]] ==$ann ? "selected" : "";
						   
									$var.='<option value="'.$ann.'" '.$selected.' >'.$ann.'</option>';
									$ann--;
								}
								$var.="</select>";
						}
							...
							$marque = array(
								...
								'Limousin' => 'Limousin',
								'Mazda' => 'Mazda',
								'MC LOUIS TANDY 640' => 'MC LOUIS TANDY 640',
								'Mercedes' => 'Mercedes',
								'Mercury' => 'Mercury',
								'Mini' => 'Mini',
							...
							$var.="<select name='".$rw['option_id']."' id='".$rw[option_id]."' class='textbox' data-native-menu='false' required />"; 
								...
								foreach($marque as $key => $val){
									$selected = $_SESSION['forms'][CFG_STAGE_ID][$rw[option_id]] ==$key ? "selected" : "";
									$var.='<option value="'.$key.'" '.$selected.'>'.$val.'</option>';	
								}
						...
						$sql_fonction=db_query("select * from tbl_category where cat_parent ='793' and cat_status='Y' order by cat_order");
						   	...
						   	$var.="<select name='".$rw[option_id]."' data-native-menu='false' />"; 
									   $var.= '<option value="">Choisir une option</option>';
							...
							$var.="<input type='text' name='".$rw[option_id]."' id='".$rw[option_id]."' value='".$_SESSION['forms'][CFG_STAGE_ID][$rw[option_id]]."' 
									 class='textbox'
									maxlength='10' maxlength='60' autocomplete='off'' required='required' >";
						}
							/*
...
function edit_option_subsub_cat($osubcatId, $classified_id){

	$sql_cat=db_query("select * from tbl_option_cat where cat_id ='$osubcatId' order by cat_id");
	$num=mysql_num_rows($sql_cat);
	$var='<table width="150%" cellpadding="2" cellspacing="0" border="0">';
	if($num > 0 ){
		$sbCatID = intval($_REQUEST['sbcatId']);
		while($rw=mysql_fetch_array($sql_cat)){
			$var.='<tr>';
			$var.='<td >'.$rw['option_nom'].':</td>';
			//------------------------------
			$sql=db_query("select * from tbl_classified_other where option_id ='$rw[option_id]' AND classifief_id=$classified_id ORDER BY other_value");
			$num2=mysql_num_rows($sql);
			...
			if($num2 > 0 ){
					$rw5=mysql_fetch_array(db_query("select * from tbl_classif_option where option_id ='$rw[option_id]' AND classif_id=$classified_id"));
					
					$var.="<select name='".$rw[option_id]."' class='textbox' required />'"; 
					$var.='<option value="">Choisir une option</option>';	
					while($rw3=mysql_fetch_array($sql)){
			   
						$selected = ($rw5[val_id] ==$rw3['val_id']) ? "selected" : "";
						
						$var.='<option value="'.$rw3['val_id'].'" '.$selected.' >'.$rw3['val_nom'].'</option>';
					}
					$var.="</select>";
				//}
			}
				...
				$var.="<select name='".$rw['option_id']."' class='textbox' required />"; 
					...
					foreach($marque as $key => $val){
						$selected = $rw2[other_value] ==$key ? "selected" : "";
						$var.='<option value="'.$key.'" '.$selected.'>'.$val.'</option>';	
					}
				...
				$var.="<select name='".$rw['option_id']."' class='textbox' required />"; 
					...
					$ann=date("Y");
					while($ann>=1900){
						$selected = $rw2[other_value] ==$ann ? "selected" : "";
			...
			$var.=$rw2[val_nom]."<input type='text' name='$rw[option_id]' value='$rw2[other_value]' class='textbox'  style='width:205px; display: -webkit-inline-box;' >";
			}
...
function contact_annonceur($act){
return $act;
