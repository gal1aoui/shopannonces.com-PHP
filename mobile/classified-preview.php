...
/**************** Display records ***************/
if(!empty($tmpClsID)){
	if(isset($_SESSION['signin']['mem_id']) && $_SESSION['signin']['mem_id']!=""){
		$sql_clsi_show="select *,DATE_FORMAT(classified_post_date ,'%d-%m-%Y') as cli_post_date
		from tbl_classified where classified_id=$tmpClsID and classified_status!='Delete' and mem_id='".$_SESSION['signin']['mem_id']."'";
	}else{
		$sql_clsi_show="select *,DATE_FORMAT(classified_post_date ,'%d-%m-%Y') as cli_post_date
		...
		while($res1=mysql_fetch_array($sql_img_set)){        
			...
			if($res1[cls_img_file]!=""){
				...
				$im_small='<a href="javascript:void(0);" onmouseover="show_gall('.$km.',7);"">
							<img src="'.$file_path_sm.'" alt="" width="65" height="65" border="0" />
							</a>';
				$im_big='
							<div class="m-item">
				...
				$joom_link[]= '<a href="'.$file_path_big.'" title="" class="link" id="gallzoom_1_'.$km.'" style="'.$dis.'" rel="lightbox[me]">
								<img src="zoom.gif" width="15" height="15" border="0" alt="" class="absm" />Agrandir la Photo</a>';
				$im_arr_big[]= $im_big;	  
				$km++;
			}
		}
